...
// Result surfaced to the UI after attempting to print via the Android bridge.
export type PrintResult = { ok: true } | { ok: false; message: string };

const BRIDGE_NOT_DETECTED_MESSAGE =
  'Printer bridge not detected. Please open the PJ Ours Printer Bridge app.';
...
const getBridgeBaseUrl = () => {
  ...
  return (process.env.NEXT_PUBLIC_THERMAL_BRIDGE_URL ?? 'http://127.0.0.1:9100').replace(/\/+$/, '');
};
...
// POSTs raw ESC/POS bytes to the Android Printer Bridge app (http://127.0.0.1:9100/print).
const sendEscPosToBridge = async (bytes: Uint8Array): Promise<PrintResult> => {
  ...
  if (
    ...
    return { ok: false, message: BRIDGE_NOT_DETECTED_MESSAGE };
  }
  ...
  try {
    ...
    if (!res.ok) {
      return { ok: false, message: `Printer bridge error (HTTP ${res.status}).` };
    }
    return { ok: true };
  } catch (err) {
...
export const printBill = async (order: PrintOrder): Promise<PrintResult> => {
  const bytes = generateEscPosBill(order);
  return sendEscPosToBridge(bytes);
};

export const printKot = async (order: PrintOrder): Promise<PrintResult> => {
  const bytes = generateEscPosKot(order);
  return sendEscPosToBridge(bytes);
};

export const printThermalBill = async (order: PrintOrder): Promise<PrintResult> => {
  return printBill(order);
};

export const printThermalKot = async (order: PrintOrder): Promise<PrintResult> => {
